...
Provar: https://github.com/takluyver/pyxdg/tree/master/test

usage: python generate_menu.py IconThemeToRender

"""
from xdg.Menu import parse, Menu, MenuEntry
from xdg.DesktopEntry import DesktopEntry
import xdg.IconTheme as theme
import os.path
import json
...
def render_svg_to_png(svg_data):
    ...
    scaleX = float(128) / float(svg.props.width)
    scaleY = float(128) / float(svg.props.height)
    ctx.save()
...
def give_me_png(filename):
  ...
  try:
    if filename[-4:] in [".png", ".jpg"]: # if is a png, return this
      return filename
      ...
      #nodejs conversion...(better than python cairo...)
      #call(["nodejs", "svg2png.js", filename, pngfile])
      svg2png(filename, pngfile)
    ...
    print "[Exception in conversion] "+str(e)
    return ""
...
def parseMenu(menu, menuJSON):
    for submenu in menu.Entries:
      if isinstance(submenu, Menu):
          ...
          iconpath=theme.getIconPath(submenu.getIcon(), 128, iconTheme) # podem fer tamany variable
          ...
          if item is not False:
            ...
            iconpath=theme.getIconPath(item.getIcon(), 128, iconTheme)
...
import json
with open(xdg_config_home+"/corellian-launcher/menu.json", 'w') as outfile:
    json.dump(catmenu, outfile)

# Prety printing final results
